...
def fetch_json(url):
    ...
    req = urllib.request.Request(
        fresh_url,
        headers={
            "User-Agent": "asx-rsi-ibkr-dry-run/1.6",
...
def signal_freshness(sig):
    ...
    if not observed:
        return False, "today signal has no permanent detection timestamp"
    try:
...
def main():
    ...
    try:
        ...
        base_url = scanner_url.rsplit("/", 1)[0]
        scanner = fetch_json(scanner_url)
        try:
            intraday = fetch_json(base_url + "/intraday_state.json")
        except Exception:
            intraday = {"latest": {}}
        try:
            entry_events = fetch_json(base_url + "/intraday_entry_events.json")
        ...
        # Permanent entry events are authoritative for fresh intraday detections.
        # Merge today's events over mutable scanner state, newest event per symbol wins.
        today = datetime.now(SYDNEY).date().isoformat()
        today_events = []
        for event in entry_events:
            if not isinstance(event, dict) or str(event.get("entry_date", "")) != today:
                continue
            sym = str(event.get("symbol", "")).upper()
            if not sym:
                continue
            today_events.append(event)
            key = f"{sym}|{event.get('entry_date', '')}"
            active_by_symbol[sym] = enriched_signal(event, stock_map.get(sym), live_map.get(key))
        ...
        print("RSI -> IBKR DRY RUN — PERMANENT-EVENT SELECTOR")
        print("CONNECTED:", ib.isConnected())
        print("ACCOUNT:", expected or accounts)
        print("POSITIONS:", sorted(positions))
        print("OPEN ORDER SYMBOLS:", sorted(open_symbols))
        print("SCANNER GENERATED AT:", scanner.get("generated_at"))
        print("SCANNER ACTIVE SIGNALS:", len(active))
        print("PERMANENT ENTRY EVENTS TOTAL:", len(entry_events))
        print("PERMANENT ENTRY EVENTS TODAY:", len(today_events))
        ...
        if not ranked:
            print("No fresh permanent entry event currently satisfies every entry rule.")
        ...
        print("--- REJECTION SUMMARY (first 20) ---")
        for sym, reason in rejected[:20]:
